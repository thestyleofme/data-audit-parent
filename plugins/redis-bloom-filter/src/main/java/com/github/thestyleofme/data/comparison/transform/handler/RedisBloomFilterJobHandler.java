...
public class RedisBloomFilterJobHandler implements BaseTransformHandler {
    @Override
    public HandlerResult handle(ComparisonJob comparisonJob, Map<String, Object> env, Map<String, Object> transformMap) {
        return null;
    }
//
...
//        String sourcePk =
//                jobEnv.getJoinMapping().stream()
//                        .findFirst()
//                        .map(mapping->BeanUtils.map2Bean(mapping,ColMapping.class))
//                        .map(ColMapping::getSourceCol)
//                        .orElseThrow(()->new HandlerException("hdsp.xadt.error.pk.not_found"));
